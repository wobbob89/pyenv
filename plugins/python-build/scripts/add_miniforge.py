...
def create_spec(filename, sha, url):
    flavor_with_suffix, version, subversion, os, arch = filename.replace(
        ".sh", ""
    ).split("-")
...
def add_version(release):
    ...
    shas = dict(
        [
            ...
            and supported(os.path.basename(name))
            and tag_name in name
        ]
    )
    specs = [
        create_spec(filename, sha, download_urls[filename])
    ...
    for distribution in DISTRIBUTIONS:
        distribution_specs = [
            spec for spec in specs if distribution in spec["flavor"].lower()
        ]
        ...
        if count > 0:
            ...
            logger.info(
                "Writing %(count)d specs for %(distribution)s to %(output_file)s",
                locals(),
            )

            script_str = install_script_fmt.format(
                install_lines="\n".join(
                    [install_line_fmt.format_map(s) for s in distribution_specs]
                ),
...
for release in requests.get(
    f"https://api.github.com/repos/{MINIFORGE_REPO}/releases"
).json():
    ...
    if any(
        not list(out_dir.glob(f"{distribution}*-{version}"))
        for distribution in DISTRIBUTIONS
    ):
